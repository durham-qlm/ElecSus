...
if __name__ =="__main__": # This line is needed for multiprocessing to work
    ...
    if (len(sys.argv) > 1): #If user specifies a run card name.
        ...
        import runcard as G
    reload(G) #Ensures the most up to date run card is used
    ...
    print '\n'
    banner = open(os.path.join(cmd_subfolder,'banner'),'r')
    ...
    SHIB,GAM,GAMB,PRE,TRA,SPE,DAT,SMB,SMF,FT,DStart,DStop,PB,SB,PFT,PHB =\
    ...
    if FT == 'No fit, just theory':
        ...
        if FT == 'Marquardt-Levenberg':
            ...
            if BFB:
                evaluationNumber += 5 #B field is harder to fit, so needs more.
            optParams, Spec = FR.RRFit(X,ydata,parameters,paramBoolList,
                                       evaluationNumber)
    ...
    # Plot results
    if FT == 'No fit, just theory':
        plotOutput(X,Spec,SPE,0,PBool=PB,path=PlotName,stickBool=PHB,ele=ELE,
                   dline=TRA)
    else:
        plotOutput(X,Spec,SPE,ydata,PB,True,residuals,PlotName,stickBool=PHB,
                   ele=ELE,dline=TRA)
