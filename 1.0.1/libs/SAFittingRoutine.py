...
def SAFit(xdata,ydata,initParams,paramBools):
    ...
    if paramBools[0]:
        errBfield     = Bfield/50.0 + 1.0
    else:
        errBfield     = 0.0

    if paramBools[1]:
        errNTemp      = (NTemp+273.15)/100.0 + 0.1
    else:
        errNTemp      = 0.0

    if paramBools[2]:
        errcellLength = cellLength/100.0
    else:
        errcellLength = 0.0
    ...
    if paramBools[4]:
        errDopTemp    = (DopTemp+273.15)/100.0 + 0.1
    else:
        errDopTemp    = 0.0
    ...
    if paramBools[7]:
        errShift      = Shift/8.0 + 0.3
    else:
        errShift      = 0.0
        
    if paramBools[8]:
        errGamma      = Gamma/10.0 + 0.5
    else:
        errGamma      = 0.0
    ...
    noEvals = 150 # Increase for higher scaling parameter accuracy
    ...
    spread = N.std(costValues, axis=0)[1]
    
    costValue = costValues[0][1] # starting point

    T = 999.0
    k = abs(spread)/T
    Beta = 0.000005 # This value should be small. Smaller gives better fit, but takes longer.
    breakThreshold = 100 # Number of times a jump uphill is rejected before switching to ML fit.
    ...
    while T > Tthreshold: 
        ...
        if paramBools[2]:
            print 'Cell length           = ', trycellLength, 'mm'
        if paramBools[3]:
            print 'Rubidium-85 fraction  = ', tryRb85, '%'
        if paramBools[4]:
            print 'Doppler temperature   = ', tryDopTemp, 'degrees Celsius'
        if paramBools[5]:
            print 'Polarization angle    = ', tryTheta0, 'degrees'
        if paramBools[6]:
            print 'Sigma minus fraction  = ', tryPol, '%'
        if paramBools[7]:
    ...
    try:
        Poptimal, Spectr, Pcov, errmsg = ML.MLfit(xdata,ydata,bestParams,paramBools)
        print errmsg
        newCostValue = Cost(y,Spectr)
        if newCostValue < bestCostValue:
            bestFitParams = Poptimal
            FinalTheory = Spectr
        else:
            print 'Downhill fit failed to find further improvements.\n'
        ...
        print 'Downhill fit failed to converge.\n'
        FinalTheory = spectrum(x,initParams[0],initParams[1],bestFitParams[0],
                               bestFitParams[1],bestFitParams[2],bestFitParams[3],
                               bestFitParams[4],bestFitParams[5],bestFitParams[6],
                               bestFitParams[7],bestFitParams[8],initParams[11],
