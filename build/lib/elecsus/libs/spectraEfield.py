...
def spectrum(
        ...
        DoppTemp=95,Constrain=True,
        SkipVoigt=False, #if true, don't add Doppler effect (for thin cells)
    ...
    if (OutputType=='E' and type(PolVector)==None):
        print 'WARNING - THERE WILL BE AN ERROR - NO INITIAL POLARISATION VECTOR DEFINED'
...
#Calculate Voigt
    ...
    if OutputType=='E':
        ...
        E_L_out = PolVector[0]*exp(1.j*total_n_Left*wavenumber*lcell)
        E_R_out = PolVector[1]*exp(1.j*total_n_Right*wavenumber*lcell)
        E_out = [E_L_out, E_R_out]
        return E_out
    elif OutputType=='Chi':
        return totalChiLeft, totalChiRight    
    elif OutputType in ['S0','Transmission','Transmission (S0)']:
    ...
    # calc outputs
        ...
        dnWRTv = derivative(d,total_n_Right.real)
        GIplus = total_n_Right.real + (X + transitionConst.v0*1.0e-6)*dnWRTv
        dnWRTv = derivative(d,total_n_Left.real)
        GIminus = total_n_Left.real + (X + transitionConst.v0*1.0e-6)*dnWRTv
