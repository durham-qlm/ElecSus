...
class FittingThread(threading.Thread):
	...
	def run(self):
		...
		## could use wx.CallAfter here as well ...		
...
##
	...
	def SaveTheoryCurves(self,filename):
		xy_data = zip(self.x_array,self.y_arrays[0],self.y_arrays[1],self.y_arrays[2],self.y_arrays[3],\
			...
			self.y_arrays[6][0], self.y_arrays[6][1], self.y_arrays[7])
...
##
	...
	def OnFileOpen(self,event):
		...
		if dlg_choice.ShowModal() == wx.ID_OK:
			...
			self.choice_index = self.plot_output_indices.index(choice)
			...
			if dlg_open.ShowModal() == wx.ID_OK:
				...
				self.fit_datatype = OutputTypes[self.choice_index]
	...
	def OnCreateResidualPlot(self,fig,canvas):
		...
		if normalised_residuals:
			### not done yet! -- requires error bars in imported data
			residuals = 100*(self.y_fit_array-self.y_arrays[self.choice_index])
		else:
			residuals = 100*(self.y_fit_array-self.y_arrays[self.choice_index])
		...
		ax_main.plot(self.x_array,self.y_arrays[self.choice_index])
...
##
	...
	def Call_ElecSus(self,calc_or_fit):
		...
		if calc_or_fit == 'Theory':
			...
			spectrum_data = elecsus_new.calculate(xrange,self.params)
